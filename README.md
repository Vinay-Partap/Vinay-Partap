<div align="center">

# Hi, I'm Vinay Partap Singh 👋

### Full-Stack Developer • AI/ML Engineer • Backend Systems

🎓 **B.Tech Computer Science — K.R. Mangalam University | Expected 2027**  
🟢 **Seeking Software Engineering Internships — Summer 2027**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/vinay-partap-singh-b8b34232a/)
[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:bhardwajvinay400@gmail.com)
[![Resume](https://img.shields.io/badge/Resume-View-111111?style=for-the-badge&logo=adobeacrobatreader&logoColor=white)](./Vinay_SDE_Resume.pdf)

</div>

---

## 👨‍💻 About Me

I'm a Computer Science student at **K.R. Mangalam University**, graduating in 2027, focused on building full-stack applications, backend systems, and AI-powered products.

- 🚀 Building **DevBattles**, an AI-powered coding learning and competitive programming platform
- ⚙️ Focused on **backend engineering, microservices, system design, cloud, and AI/LLM applications**
- ☁️ Working with **AWS, Docker, PostgreSQL, Redis, and CI/CD**
- 🟢 Open to **Software Engineering internships for Summer 2027**

---

## 🚀 Featured Projects

### 🏆 DevBattles — AI-Powered Coding Learning & Competitive Programming Platform

A full-stack coding platform combining structured learning, competitive programming, and AI-powered coding challenges and evaluation.

[🌐 **Live Demo**](https://devbattle-frontend-kappa.vercel.app/)

**Highlights**
- Microservice-oriented backend with dedicated authentication and question services
- JWT-based authentication and RBAC
- PostgreSQL + Redis data layer
- Supports DSA, JavaScript, SQL, React, HTML, CSS, Backend, System Design, and AI challenges
- Structured evaluation configuration for different question types
- Private evaluation configuration and public question data separation
- Docker-based development and production-oriented architecture

**Stack:** React • TypeScript • Node.js • PostgreSQL • Redis • Docker • AWS • AI/LLMs

---

### 👁️ VisionGuard — Real-Time Vision & Safety System

Real-time computer vision system for pedestrian and vehicle detection with proximity-based risk classification and configurable alerts.

[🌐 **Live Demo**](https://visionguard-dwiz4twnypak6dkqkr96zz.streamlit.app/)

**Highlights**
- YOLOv8-based object detection
- Real-time video processing with OpenCV
- Pedestrian and vehicle detection
- Proximity-based risk classification
- Configurable risk-aware alert logic

**Stack:** Python • YOLOv8 • PyTorch • OpenCV • Streamlit

---

### 🗄️ CipherSQLStudio — Browser-Based SQL Learning Platform

Interactive SQL learning environment designed around hands-on query execution and PostgreSQL database practice.

[🌐 **Live Demo**](https://cipher-sql-studio-mu.vercel.app/)

**Highlights**
- Interactive SQL execution
- PostgreSQL sandbox environment
- Transaction-safe query execution
- Query result visualization
- Structured SQL learning workflow
- AI-assisted learning features

**Stack:** JavaScript • Node.js • Express.js • PostgreSQL • AI/LLMs

---

## 🛠️ Tech Stack

### Languages

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat-square&logo=postgresql&logoColor=white)

### Frontend

![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST_APIs-02569B?style=flat-square)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=flat-square&logo=jsonwebtokens&logoColor=white)
![RBAC](https://img.shields.io/badge/RBAC-4B5563?style=flat-square)

### Databases & Cloud

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)

### AI / ML

![LangGraph](https://img.shields.io/badge/LangGraph-1C1C1C?style=flat-square)
![Google Gemini](https://img.shields.io/badge/Google_Gemini-4285F4?style=flat-square&logo=googlegemini&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![YOLOv8](https://img.shields.io/badge/YOLOv8-111111?style=flat-square)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)

### DevOps & Engineering

![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)
![PM2](https://img.shields.io/badge/PM2-2B037A?style=flat-square)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white)
![Render](https://img.shields.io/badge/Render-46E3B7?style=flat-square&logo=render&logoColor=black)

**Core:** DSA • OOP • DBMS • System Design • Microservices • SDLC • Agile • ACID • Secure Authentication • RBAC • Rate Limiting • Schema Design • CI/CD

---

## 🏆 Certifications & Recognition

| Achievement | Organization | Year |
|---|---|---:|
| AWS AI Practitioner Learning Plan | Amazon Web Services | 2026 |
| McKinsey Forward Program | McKinsey & Company | 2025 |
| Startup School: Prompt to Prototype | Google for Startups | 2025 |
| BCG GenAI Job Simulation | BCG X | 2025 |
| AWS APAC Solutions Architecture Simulation | Forage / AWS | 2025 |
| Google Campus Ambassador | Google | 2025 |
| Dean's List Scholar | K.R. Mangalam University | 2025 |

---

## 📚 Currently Learning

- Distributed systems and scalable backend architecture
- Kubernetes and container orchestration
- Advanced LLM application architecture
- Cloud-native deployment and observability

---

## 🤝 Contact

📧 **Email:** YOUR_EMAIL  
💼 **LinkedIn:** [Vinay Partap Singh](YOUR_LINKEDIN_URL)

🟢 **Open to Software Engineering Internships — Summer 2027**

---

<div align="center">

⭐ If you find my projects useful, consider starring the repositories!

</div>
